...
def get(argv, mode='scaling_data'):
    ...
    output_dir = 'output'
    if '512' in name: # HACK - for distributed benchmarks TODO: find better solution
        output_dir += '_distributed'
    if mode == 'scaling_data':
        exp_dict[BM_OUTPUT_FILENAME] = output_dir + '/contour_' + algorithm + '/contour_data_' + name + '.csv'
    if mode == 'benchmark':
        exp_dict[BM_OUTPUT_FILENAME] = output_dir + '/contour_' + algorithm + '/contour_weak_' + name + '.csv'
    exp_dict[BM_ALGO] = 'contour'
    ...
    if '512' in name: # HACK - for distributed benchmarks TODO: find better solution
        exp_dict[SEARCH_DATA_ORIGINAL_DIM] = [512]*3
    exp_dict[SEARCH_REPLICATION_SCALING_MODE] = 'box'

    exp_dict[IC_ALGORITHM] = algorithm
    exp_dict[IC_VALUES] = [0,64,128,192,255]
