...
def get(argv, mode='scaling_data'):
    ...
    if mode == 'scaling_data':
        exp_dict[BM_OUTPUT_FILENAME] = 'output/volumerender/volumerender_data_' + name + '.csv'
    if mode == 'benchmark':
        exp_dict[BM_OUTPUT_FILENAME] = 'output/volumerender/volumerender_weak_' + name + '.csv'
    exp_dict[BM_ALGO] = 'volumerender'
    exp_dict[BM_NUM_REP] = 1
    exp_dict[OUT_DATA_PREFIX] = path + '/' + exp_dict[BM_ALGO] + '_' + file[:-4]
    ...
    exp_dict[SEARCH_DATA_ORIGINAL_DIM] = [256]*3

    # this is necessary so that all data takes up the same amount of screen space
    exp_dict[SEARCH_REPLICATION_SCALING_MODE] = 'box' # valid entries: 'box', 'line'

    exp_dict[VR_IMAGE_SIZE] = [800,800]
    exp_dict[VR_ERT] = True
    exp_dict[VR_OPACITY] = 0.02
